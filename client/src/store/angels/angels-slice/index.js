...
export const fetchFilteredAngels = createAsyncThunk(
  'angels/angels/get',
  async ({ filter, sort }) => {
    ...
    const res = await axios.get(
      `${API_URL}/api/angels/angels/get?${queryParams}`
    )
    return res?.data
  }
)
...
const angelsFilterSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchFilteredAngels.pending, (state) => {
      ...
      .addCase(fetchFilteredAngels.rejected, (state, action) => {
        ;((state.angelsList = []), (state.isLoading = false))
      })
  }
})

export const likeAngel = createAsyncThunk('angels/angels/like', async (id) => {
  const { angelId } = id

  const res = await axios.post(
    `${API_URL}/api/angels/angels/like/${angelId}`,
    id,
    {
...
export const unlikeAngel = createAsyncThunk(
  'angels/angels/unlike',
  async (id) => {
...
const angelLikesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(likeAngel.pending, (state, action) => {
        state.isLoading = true
      })
...
export const addComment = createAsyncThunk(
  'angels/angels/comments/',
  async (id) => {
    const { angelId, userId, commentText } = id
    const res = await axios.post(
      `${API_URL}/api/angels/angels/comments/${angelId}`,
      id
    )
    return res?.data
  }
